...
<div class="row">
	...
	<div class="col-md-4">
						...
						<form method="post" action="">
                                        <select name="university_number" class="select2_single form-control" required="required" tabindex="-1" >
										<option value="0">Select university ID Number</option>
										<?php
										$result= mysqli_query($con,"select * from user where status = 'Active' ") or die (mysqli_error());
										while ($row= mysqli_fetch_array ($result) ){
										$id=$row['user_id'];
										?>
                                            <option value="<?php echo $row['university_number']; ?>"><?php echo $row['university_number']; ?> - <?php echo $row['firstname']; ?></option>
										<?php } ?>
                                        </select>
				<br />
				<br />
...
<?php
	...
	$row = mysqli_fetch_array($sql);
		...
		if($count <= 0){
			...
			$university_number = $_POST['university_number'];
			echo ('<script> location.href="borrow_book.php?university_number='.$university_number.'";</script');
		}
	}
?>
