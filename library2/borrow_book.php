...
<?php 
	$university_number = $_GET['university_number'];
	
	$user_query = mysqli_query($con,"SELECT * FROM user WHERE university_number = '$university_number' ");
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            <div class="x_title">
                ...
                <h2>
                    Borrower Name : <span
                        style="color:maroon;"><?php echo $row['firstname']." ".$row['middlename']." ".$row['lastname']; ?></span>
                </h2>
            ...
            <div class="x_content">
                ...
                <div class="table-responsive">
                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered"
                        ...
                        <tbody>
							...
							?>
                            <script>
                            window.location = "borrow_book.php?university_number=<?php echo $university_number ?>";
                            </script>
                            <?php 
																}
							?>
                ...
                <div class="row" style="margin-top:30px;">
                    <form method="post">
                        <div class="col-xs-4">
                            <input type="text" style="margin-bottom:10px; margin-left:-9px;" class="form-control"
                                name="barcode" placeholder="Enter book name here....." autofocus required />
                        </div>
                    </form>
                        ...
                        <form method="post" action="">
                            ...
                            <?php 
								if (isset($_POST['barcode'])){
									...
									$book_query = mysqli_query($con,"SELECT * FROM book WHERE book_title = '$barcode' ") or die (mysqli_error());
									...
									if ($book_count == 0){
										echo '
											<table>
												<tr>
													<td class="alert alert-info">No match for the name entered!</td>
												</tr>
											</table>
										';
									} elseif ($barcode == '') {
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <img src="upload/<?php echo $book_row['book_image']; ?>" width="100px"
                                        height="100px" style="border:4px groove #CCCCCC; border-radius:5px;">
                                    <?php else: ?>
                                ...
                                <td><button name="borrow" class="btn btn-info"><i class="fa fa-check"></i>
                                        Borrow</button></td>
                            </tr>
                            <?php } }?>
                            ...
                            <?php 
								if (isset($_POST['borrow'])){
									...
									if ($countBorrowed['books_allowed'] == $allowed['qntty_books']){
										echo "<script>alert(' ".$allowed['qntty_books']." ".'Books Allowed per User!'." '); window.location='borrow_book.php?university_number=".$university_number."'</script>";
									}elseif ($bookCount['book_count'] == 1){
										echo "<script>alert('Book Already Borrowed!'); window.location='borrow_book.php?university_number=".$university_number."'</script>";
									}else{
									...
									if ($new_book_copies < 0){
										echo "<script>alert('Book out of Copy!'); window.location='borrow_book.php?university_number=".$university_number."'</script>";
									}elseif ($copies_row['status'] == 'Damaged'){
										echo "<script>alert('Book Cannot Borrow At This Moment!'); window.location='borrow_book.php?university_number=".$university_number."'</script>";
